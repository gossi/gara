...
gara.Class("gara.widgets.Button", function() { return /** @lends gara.widgets.Button# */ {
	...
	$constructor : function (parent, style) {
		...
		if (!((style & gara.PUSH) === gara.PUSH
				|| (style & gara.RADIO) === gara.RADIO
				|| (style & gara.CHECK) === gara.CHECK)) {
			style |= gara.PUSH;
		}

		// make default orientation
		if (!((style & gara.VERTICAL) === gara.VERTICAL
				|| (style & gara.HORIZONTAL) === gara.HORIZONTAL)) {
			style |= gara.HORIZONTAL;
		}
	 ...
	 * @param {gara.events.SelectionListener} listener the desired listener to be added to this list
	...
	createWidget : function () {
		...
		this.setClass("garaButtonPush", (this.style & gara.PUSH) === gara.PUSH);
		this.setClass("garaButtonHorizontal", (this.style & gara.HORIZONTAL) === gara.HORIZONTAL);
		this.setClass("garaButtonVertical", (this.style & gara.VERTICAL) === gara.VERTICAL);
	...
	getSiblingRadioButtons : function () {
		...
		// parent is dom node
			...
			for (i = 0, len = childs.length; i < len; i++) {
				child = childs[i];
				if (child.widget && child.widget instanceof gara.widgets.Control) {
		...
		for (i = index - 1; i >= 0 && controls[i] instanceof gara.widgets.Button && (controls[i].getStyle() & gara.RADIO) === gara.RADIO; --i) {
			buttons.unshift(controls[i]);
		}

		for (i = index + 1; i < controls.length && controls[i] instanceof gara.widgets.Button && (controls[i].getStyle() & gara.RADIO) === gara.RADIO; ++i) {
	...
	handleMouseEvents : function (e) {
		...
		case "mousedown":
			...
			this.pressEvent = gara.addEventListener(document, "mouseup", this);
			break;
		...
		case "mouseup":
			...
			if (e.target.widget && e.target.widget === this) {
				this.setSelection((this.style & gara.RADIO) === gara.RADIO ? true : !this.getSelection());
			}
			break;
		}
	},
	 ...
	 *
	 * @param {gara.events.SelectionListener} listener the listener to remove from this list
	 * @return {void}
	 */
	...
	setSelection : function (selected) {
		...
		if (((this.style & gara.CHECK) === gara.CHECK ||
				(this.style & gara.RADIO) === gara.RADIO)
				&& this.enabled) {
			...
			if ((this.style & gara.RADIO) === gara.RADIO && selected) {
				buttons = this.getSiblingRadioButtons();
				current = buttons.indexOf(this);
				prev = current - 1;
				next = current + 1;
