...
(function() {
	...
	// inheritance members
		...
		useqs = {},
		childs = {},
		ccbc = 1, fireChain, fireReady, fixContexts, fixDescriptor,
		Class = function () {};
	...
	gara.generateUID = function () {
		var d = new Date();
		return "garaID" + d.getDay() + d.getHours() + d.getMinutes() + d.getSeconds() + d.getMilliseconds();
	};
	...
	gara.setResourcePath = function (resource, path) {
	...
	Class.extend = function (prop) {
		var $super = this.prototype, prototype, protectedProps = {}, key;
	...
	gara.use = function(names) {
		...
		names = typeof(names) === "string" ? [names] : names;
		names.forEach(function (name) {
			if (!Object.prototype.hasOwnProperty.call(useqs, useClass)) {
				useqs[useClass] = [];
			}
			useqs[useClass].push(name);
		});
	};

	gara.parent = function (name, callback) {
		gara.ls(name);
		gara.use(name);
