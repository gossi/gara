...
$class("TabFolder", {
	...
	_addItem : function(item) {
		this.checkWidget();
		if (!$class.instanceOf(item, gara.jswt.widgets.TabItem)) {
	...
	_create : function() {
		...
		if ((this._style & JSWT.DROP_DOWN) == JSWT.DROP_DOWN) {
			...
			this._dropDownMenu = new gara.jswt.widgets.Menu(this, JSWT.DROP_DOWN);
...
//		return null;
	...
	handleEvent : function(e) {
		...
		e.item = widget && $class.instanceOf(widget, gara.jswt.widgets.TabItem) ? widget : this._activeItem;
		e.widget = this;
		this._event = e;
	...
	_handleMouseEvents : function(e) {
		switch (e.type) {
			case "mousedown":
				...
				if ($class.instanceOf(e.target.widget, gara.jswt.widgets.MenuItem)) {
	...
	toString : function() {
		return "[gara.jswt.widgets.TabFolder]";
	},
