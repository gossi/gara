...
$class("MenuItem", {
	...
	_create : function() {
		...
		if ((this._style & JSWT.SEPARATOR) == JSWT.SEPARATOR) {
			...
			this.handle.setAttribute("aria-haspopup", ((this._style & JSWT.CASCADE) == JSWT.CASCADE) && this._menu != null);

			// css
			this.setClass("jsWTMenuItemCascade", ((this._style & JSWT.CASCADE) == JSWT.CASCADE) && this._menu != null);
			this.setClass("jsWTMenuItemCheck", (this._style & JSWT.CHECK) == JSWT.CHECK);
			this.setClass("jsWTMenuItemRadio", (this._style & JSWT.RADIO) == JSWT.RADIO);
		...
		if (this._enabled &&
			...
			if ((this._style & JSWT.RADIO) == JSWT.RADIO) {
				if (!this._parent._managingRadioChecks) {
					...
					var i = pos - 1;
					while (i >= 0 && (this._parent.getItem(i).getStyle() & JSWT.RADIO) == JSWT.RADIO) {
						this._parent.getItem(i--).setSelection(false);
					}

					// removing selection downwards
					i = pos + 1;
					while (i < this._parent.getItems().length && (this._parent.getItem(i).getStyle() & JSWT.RADIO) == JSWT.RADIO) {
						this._parent.getItem(i++).setSelection(false);
					}
	...
	setText : function(text) {
		...
		if (this.handle && (this._style & JSWT.SEPARATOR) != JSWT.SEPARATOR) {
	...
	toString : function() {
		return "[gara.jswt.widgets.MenuItem]";
	},
