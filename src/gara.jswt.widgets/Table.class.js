...
$class("Table", {
	...
	_addColumn : function(column, index) {
		this.checkWidget();
		if (!$class.instanceOf(column, gara.jswt.widgets.TableColumn)) {
	...
	_create : function() {
		...
		this.handle.setAttribute("aria-multiselectable", (this._style & JSWT.MULTI) == JSWT.MULTI ? true : false);
		...
		for (var eventType in this._listener) {
			this._listener[eventType].forEach(function(listener) {
	...
	handleEvent : function(e) {
		...
		e.item = widget && ($class.instanceOf(widget, gara.jswt.widgets.TableItem) || $class.instanceOf(widget, gara.jswt.widgets.TableColumn)) ? widget : this._activeItem;
		e.widget = this;
		this._event = e;
	 ...
	 * @throws {gara.jswt.widgets.ItemNotExistsException} if the item does not exist in this table
	...
	selectRange : function(from, to) {
		if ((to - from) > 1 && (this._style & JSWT.MULTI) == JSWT.MULTI) {
			for (var i = from; i <= to; i++) {
	...
	toString : function() {
		return "[gara.jswt.widgets.Table]";
	},
