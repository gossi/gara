...
$class("TableColumn", {
		...
		switch(e.type) {
			case "mousedown":
				...
				if (e.target == this.handle && this._moveable) {
					...
					this._shadow = new gara.jswt.widgets.Table(document.getElementsByTagName("body")[0], this._table.getStyle() &~ JSWT.CHECK);
			...
			case "mouseup":
				...
				if (this._isMoving) {
					...
					if (e.target.widget && $class.instanceOf(e.target.widget, gara.jswt.widgets.TableColumn)
	...
	toString : function() {
		return "[gara.jswt.widgets.TableColumn]";
	},
